...
// ✅ Works for BrowserRouter (?id=93) AND HashRouter (#/ProductDetail?id=93)
...
// OPTIONAL: store item for your “original UI cart” (localStorage cart)
function addToLocalCart(product, quantity) {
  const key = "cart";
  const existing = JSON.parse(localStorage.getItem(key) || "[]");

  const pid = Number(product.id);
  const found = existing.find((i) => Number(i.product_id) === pid);
...
export default function ProductDetail() {
  ...
  // ✅ Map related to match your ProductCard expectations
  ...
  // ✅ Proxy fields
  const image = product.image || ""; // NOT product.images
